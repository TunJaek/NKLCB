...
<div ng-app="coolform" class="wrapper">
  <div cool-form>
    ...
    <div class="q-after">
      <hr>
      <div class="q-confirm-text">선택하신 항목이 맞으십니까? (가구는 구매 완료된 상태여야 합니다.)</div>
...
coolform.directive('coolForm', function($timeout) {
    var ctrl = function($scope, $element, $attributes) {
      ...
      $scope.questions.push({'question': '가구의 이름을 입력해주세요.'});
      $scope.questions.push({'question': '가구 수량을 입력해주세요.'});
      $scope.questions.push({'question': '가구의 금액대는 얼마인가요?'});
      $scope.questions.push({'question': '서비스 희망 날짜를 입력해주세요.'});
      $scope.questions.push({'question': '가구의 출발 지역을 입력해주세요.'});
      $scope.questions.push({'question': '가구 관련 특이사항을 입력해주세요.'});
      $scope.questions.push({'question': '가구 도착 지역을 입력해주세요.'});
