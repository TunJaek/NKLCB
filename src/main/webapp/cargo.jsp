...
<div ng-app="coolform" class="wrapper">
  ...
  <div cool-form>
    <div class="q-title">용달•화물 운송</div>
...
coolform.directive('coolForm', function($timeout) {
    var ctrl = function($scope, $element, $attributes) {
      ...
      $scope.questions.push({'question': '짐은 포장된 상태여야 합니다.(네/아니오)'});
      $scope.questions.push({'question': '운전만 필요하신가요?'});
      $scope.questions.push({'question': '운전 및 물품배달까지 필요하신가요?'});
      $scope.questions.push({'question': '서비스 희망일을 입력해주세요'});
      $scope.questions.push({'question': '시간대를 입력해주세요'});
      $scope.questions.push({'question': '출발지는 어디인가요?(주소)'});
      $scope.questions.push({'question': '출발지에 엘레베이터가 있나요?'});
      $scope.questions.push({'question': '도착지는 어디인가요?(주소)'});
      $scope.questions.push({'question': '도착지에 엘레베이터가 있나요?'});
      $scope.questions.push({'question': '짐의 종류와 수량을 입력해주세요.'});
