...
coolform.directive('coolForm', function($timeout) {
    var ctrl = function($scope, $element, $attributes) {
      ...
      $scope.questions.push({'question': '청소가 필요한곳을 입력해주세요'});
      $scope.questions.push({'question': '집은 몇 평인가요?'});
      $scope.questions.push({'question': '청소 시간을 입력해주세요'});
      $scope.questions.push({'question': '청소 시작 시간대를 입력해주세요'});
      $scope.questions.push({'question': '정기적인 청소를 원하시나요?'});
      $scope.questions.push({'question': '고객님의 성별은 무엇인가요?'});
